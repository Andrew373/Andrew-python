...
class ConstantSumRowWithColMaxesIterator(object):
    ...
    def _iterate_rows(self, prev_row, prev_idx=0):
        for i in range(prev_idx, self.ncol):
...
#for M in ConstantMarginalMatrixIterator(row_mar=[8, 10, 12], 
                                      # col_mar=[4, 9, 5, 12]):
    #write.table(M, file = "Results.csv", append = T)
    
    
Results = ConstantMarginalMatrixIterator(row_mar=[8, 10, 12], col_mar=[4, 9, 5, 12])
